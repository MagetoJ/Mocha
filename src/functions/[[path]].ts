...
// Define the environment bindings for your code.
// Cloudflare Pages automatically provides these.
interface Env {
  DB: D1Database;
  R2_BUCKET: R2Bucket;
}
...
app.post("/api/login", async (c) => {
  ...
  const staff: any = await c.env.DB.prepare("SELECT * FROM staff WHERE email = ? AND is_active = 1").bind(email).first();
  if (!staff || !staff.password) { return c.json({ error: "Invalid credentials" }, 401); }
  ...
  return c.json({ success: true, user: { email: staff.email, staff: staffInfo } });
});
...
app.get("/api/menu/categories", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM menu_categories WHERE is_active = 1 ORDER BY display_order, name").all();
  return c.json(results);
});

app.post("/api/menu/categories", async (c) => {
  const { name, description, display_order } = await c.req.json();
  const result = await c.env.DB.prepare( `INSERT INTO menu_categories (name, description, display_order, updated_at) VALUES (?, ?, ?, datetime('now'))` ).bind(name, description || null, display_order || 0).run();
...
app.put("/api/menu/categories/:id", async (c) => {
  ...
  await c.env.DB.prepare( `UPDATE menu_categories SET name = ?, description = ?, display_order = ?, is_active = ?, updated_at = datetime('now') WHERE id = ?` ).bind(name, description || null, display_order || 0, is_active !== undefined ? (is_active ? 1 : 0) : 1, categoryId).run();
  return c.json({ success: true });
});
...
app.get("/api/menu/items", async (c) => {
  const { results } = await c.env.DB.prepare(` SELECT mi.*, mc.name as category_name FROM menu_items mi JOIN menu_categories mc ON mi.category_id = mc.id WHERE mi.is_available = 1 AND mc.is_active = 1 ORDER BY mc.display_order, mi.name `).all();
  return c.json(results);
});

app.post("/api/menu/items", async (c) => {
  const { category_id, name, description, price, image_url, preparation_time } = await c.req.json();
  const result = await c.env.DB.prepare( `INSERT INTO menu_items (category_id, name, description, price, image_url, preparation_time, updated_at) VALUES (?, ?, ?, ?, ?, ?, datetime('now'))` ).bind(category_id, name, description || null, price, image_url || null, preparation_time || 15).run();
  return c.json({ id: result.meta.last_row_id, success: true });
});
...
app.put("/api/menu/items/:id", async (c) => {
  ...
  await c.env.DB.prepare( `UPDATE menu_items SET category_id = ?, name = ?, description = ?, price = ?, image_url = ?, preparation_time = ?, is_available = ?, updated_at = datetime('now') WHERE id = ?` ).bind(category_id, name, description || null, price, image_url || null, preparation_time || 15, is_available !== undefined ? (is_available ? 1 : 0) : 1, itemId).run();
  return c.json({ success: true });
});
...
app.get("/api/tables", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM tables ORDER BY room_name, table_number").all();
  return c.json(results);
});
...
app.get("/api/performance/staff", async (c) => {
  const { results } = await c.env.DB.prepare(`
    SELECT 
      s.id, s.employee_id, s.first_name, s.last_name, s.role, sp.date, sp.orders_served, sp.total_sales, sp.tables_served, sp.shift_duration_minutes, sp.customer_rating_avg, sp.tips_earned,
...
app.get("/api/performance/staff/:id", async (c) => {
  const staffId = c.req.param("id");
  const { results } = await c.env.DB.prepare(`
    SELECT s.id, s.employee_id, s.first_name, s.last_name, s.role, sp.date, sp.orders_served, sp.total_sales, sp.tables_served, sp.shift_duration_minutes, sp.customer_rating_avg, sp.tips_earned
    FROM staff s
    LEFT JOIN staff_performance sp ON s.id = sp.staff_id
...
app.get("/api/performance/summary", async (c) => {
  const todayResults = await c.env.DB.prepare(`
    SELECT COUNT(DISTINCT s.id) as active_staff, COALESCE(SUM(sp.orders_served), 0) as total_orders_today, COALESCE(SUM(sp.total_sales), 0) as total_sales_today, COALESCE(AVG(sp.customer_rating_avg), 0) as avg_rating_today
    FROM staff s LEFT JOIN staff_performance sp ON s.id = sp.staff_id AND sp.date = date('now')
    WHERE s.is_active = 1
  `).first();
  const yesterdayResults = await c.env.DB.prepare(`
    SELECT COALESCE(SUM(sp.orders_served), 0) as total_orders_yesterday, COALESCE(SUM(sp.total_sales), 0) as total_sales_yesterday
    FROM staff s LEFT JOIN staff_performance sp ON s.id = sp.staff_id AND sp.date = date('now', '-1 day')
    WHERE s.is_active = 1
  `).first();
  const topPerformers = await c.env.DB.prepare(`
    SELECT s.first_name, s.last_name, s.role, sp.total_sales, sp.orders_served, sp.customer_rating_avg
    FROM staff s JOIN staff_performance sp ON s.id = sp.staff_id
    WHERE sp.date = date('now') AND s.is_active = 1
    ORDER BY sp.total_sales DESC LIMIT 5
  `).all();
  return c.json({ today: todayResults, yesterday: yesterdayResults, topPerformers: topPerformers.results });
});

app.get("/api/performance/trends", async (c) => {
  const { results } = await c.env.DB.prepare(`
    SELECT sp.date, SUM(sp.orders_served) as total_orders, SUM(sp.total_sales) as total_sales, AVG(sp.customer_rating_avg) as avg_rating, COUNT(DISTINCT sp.staff_id) as active_staff
...
app.get("/api/performance/by-role", async (c) => {
  const { results } = await c.env.DB.prepare(`
    SELECT s.role, COUNT(DISTINCT s.id) as staff_count, COALESCE(SUM(sp.orders_served), 0) as total_orders, COALESCE(SUM(sp.total_sales), 0) as total_sales, COALESCE(AVG(sp.customer_rating_avg), 0) as avg_rating, COALESCE(SUM(sp.tips_earned), 0) as total_tips
    FROM staff s LEFT JOIN staff_performance sp ON s.id = sp.staff_id AND sp.date >= date('now', '-7 days')
    WHERE s.is_active = 1
    GROUP BY s.role ORDER BY total_sales DESC
  `).all();
  return c.json(results);
});

app.post("/api/performance/initialize", async (c) => {
  const { results: allStaff } = await c.env.DB.prepare(`SELECT id FROM staff WHERE is_active = 1`).all();
  let initializedCount = 0;
  for (const staff of allStaff) {
    const result = await c.env.DB.prepare(`
      INSERT OR IGNORE INTO staff_performance (staff_id, date, created_at, updated_at)
      VALUES (?, date('now'), datetime('now'), datetime('now'))
    `).bind(staff.id).run();
  ...
  return c.json({ success: true, message: `Initialized records for ${initializedCount} staff`, totalStaff: allStaff.length });
});

// --- RECEPTIONIST DASHBOARD ---
app.get("/api/receptionist/dashboard", async (c) => {
  const todayStats = await c.env.DB.prepare(`
    SELECT COUNT(DISTINCT gc.id) as today_checkins, COALESCE(AVG(wg.estimated_wait_minutes), 0) as avg_wait_time
    FROM guest_checkins gc LEFT JOIN waiting_guests wg ON wg.status = 'waiting'
    WHERE date(gc.checked_in_at) = date('now')
  `).first();
  const waitingGuests = await c.env.DB.prepare(`
    SELECT id, guest_name, guest_phone as phone, party_size, arrived_at, estimated_wait_minutes as estimated_wait
    FROM waiting_guests WHERE status = 'waiting' ORDER BY arrived_at ASC
  `).all();
  const reservations = await c.env.DB.prepare(`
    SELECT id, guest_name, guest_phone, guest_email, party_size, datetime(reservation_date || ' ' || reservation_time) as reservation_time, status, table_id, special_requests
    FROM reservations WHERE reservation_date = date('now') AND status IN ('confirmed', 'seated')
    ORDER BY reservation_time ASC
  `).all();
  const tableStats = await c.env.DB.prepare(`
    SELECT COUNT(*) as total_tables, SUM(CASE WHEN is_occupied = 1 THEN 1 ELSE 0 END) as occupied_tables
    FROM tables
  `).first();
  return c.json({
    stats: {
...
app.post("/api/receptionist/reservation", async (c) => {
  ...
  return c.json({ success: true, reservation_id: result.meta.last_row_id });
});
...
app.post("/api/receptionist/waiting-guest", async (c) => {
  ...
  return c.json({ success: true, waiting_guest_id: result.meta.last_row_id });
});
...
app.post("/api/receptionist/checkin", async (c) => {
  ...
  const table = await c.env.DB.prepare(`SELECT id, is_occupied FROM tables WHERE id = ?`).bind(table_id).first();
  if (!table) return c.json({ error: "Table not found" }, 404);
  if (table.is_occupied) return c.json({ error: "Table is already occupied" }, 400);
  try {
    await c.env.DB.prepare(`UPDATE tables SET is_occupied = 1, updated_at = datetime('now') WHERE id = ?`).bind(table_id).run();
    ...
    if (reservation_id) {
      await c.env.DB.prepare(`
        UPDATE reservations SET status = 'seated', table_id = ?, seated_at = datetime('now'), updated_at = datetime('now') WHERE id = ?
      `).bind(table_id, reservation_id).run();
    }
    if (waiting_guest_id) {
      await c.env.DB.prepare(`
        UPDATE waiting_guests SET status = 'seated', table_id = ?, seated_at = datetime('now'), updated_at = datetime('now') WHERE id = ?
      `).bind(table_id, waiting_guest_id).run();
    }
    return c.json({ success: true, checkin_id: checkinResult.meta.last_row_id });
  } catch (error) {
    await c.env.DB.prepare(`UPDATE tables SET is_occupied = 0, updated_at = datetime('now') WHERE id = ?`).bind(table_id).run();
...
app.post("/api/receptionist/seat-guest", async (c) => {
  ...
  if (!guestId || !tableNumber) return c.json({ error: "Guest ID and table number are required" }, 400);
  const table = await c.env.DB.prepare(`SELECT id, is_occupied FROM tables WHERE table_number = ?`).bind(tableNumber).first();
  if (!table) return c.json({ error: "Table not found" }, 404);
  if (table.is_occupied) return c.json({ error: "Table is already occupied" }, 400);
  const waitingGuest = await c.env.DB.prepare(`SELECT id, guest_name, party_size FROM waiting_guests WHERE id = ? AND status = 'waiting'`).bind(guestId).first();
  if (!waitingGuest) return c.json({ error: "Waiting guest not found or already seated" }, 404);
  try {
    await c.env.DB.prepare(`UPDATE tables SET is_occupied = 1, updated_at = datetime('now') WHERE id = ?`).bind(table.id).run();
    await c.env.DB.prepare(`
    ...
    await c.env.DB.prepare(`UPDATE tables SET is_occupied = 0, updated_at = datetime('now') WHERE id = ?`).bind(table.id).run();
...
app.get("/api/receptionist/available-tables", async (c) => {
  const { results } = await c.env.DB.prepare(`
    SELECT id, table_number, room_name, capacity, is_occupied
    FROM tables WHERE is_occupied = 0
...
// --- FINAL EXPORT ---
// This is the special export format for Cloudflare Pages Functions.
// It tells Cloudflare to use Hono to handle all incoming requests.
